...
namespace CircuitSharp.Core
{
    public class Circuit
    {
        ...
        public void Connect(Lead left, Lead right)
        {
            ...
            if (leftLeadIndex >= leftLeads.Length)
                Array.Resize(ref leftLeads, leftLeadIndex + 1);

            if (rightLeadIndex >= rightLeads.Length)
                Array.Resize(ref rightLeads, rightLeadIndex + 1);
        ...
        private void AddElement(ICircuitElement element)
        {
            if (!elements.Contains(element))
            {
                ...
                nodeMesh.Add(new long[element.GetLeadCount()]);
                ...
                for (var j = 0; j != circuitMatrixFullSize; j++)
                {
                    ...
                    if (j < nodeList.Count - 1)
                    {
                        ...
                        for (var k = 0; k != nodeMesh.Count; k++)
                        {
                            ...
                            var leads = new List<long>(nodeMesh[k]);
                            var index = leads.IndexOf(GetNodeId(j + 1));
                            if (index != -1)
